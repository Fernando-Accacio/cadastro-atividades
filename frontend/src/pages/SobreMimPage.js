...
// Texto de fallback caso o banco esteja vazio
const DEFAULT_INTRO = "Adicone sua introdução informal no painel administrativo.";

function SobreMimPage() {
  // 1. Criar estado para hobbies E introdução
  const [hobbies, setHobbies] = useState([]);
  const [introText, setIntroText] = useState(''); // <-- NOVO ESTADO
  const [isLoading, setIsLoading] = useState(true);

  // 2. Buscar TODOS os dados da API quando o componente carregar
  useEffect(() => {
    setIsLoading(true);
    
...
        ) : (
          // Usar whiteSpace: 'pre-line' para respeitar as quebras de linha do <textarea>
...
      {/* --- SEÇÃO DE HOBBIES (sem mudança no JSX, só no fetch) --- */}
